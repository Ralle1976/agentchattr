...
// Session state
let activeSession = null; // current active session for this channel
let sessionTemplates = []; // available templates
let activeSessionsByChannel = {}; // { channelName: session }
...
function appendMessage(msg) {
    ...
    if (msg.type === 'join' || msg.type === 'leave') {
        ...
        const goal = msg.metadata?.goal ? ` -- ${escapeHtml(msg.metadata.goal)}` : '';
        el.innerHTML = `<span class="session-banner-icon">&#9654;</span> <strong>${escapeHtml(msg.text)}</strong>${goal}`;
        ...
        el.innerHTML = `<span class="session-banner-icon">&#9632;</span> <strong>${escapeHtml(msg.text)}</strong>${jumpLink}`;
    } else if (msg.type === 'session_phase') {
        el.classList.add('system-msg', 'session-banner', 'session-banner-phase');
        el.innerHTML = `<span class="session-banner-icon">&#9656;</span> ${escapeHtml(msg.text)}`;
    } else if (msg.type === 'session_draft') {
        ...
        if (isSuperseded) {
            ...
            el.innerHTML = `
                <div class="session-draft-body">
                    <div class="session-draft-header">Draft from ${escapeHtml(proposedBy)} (invalid)</div>
                    <ul class="session-draft-errors">${errorList}</ul>
                    <div class="session-draft-actions">
                        <button class="session-draft-btn changes" onclick="requestDraftChanges('${escapeHtml(draftId)}', '${escapeHtml(proposedBy)}', ${msg.id})">Request Changes</button>
                        <button class="session-draft-btn dismiss" onclick="dismissDraft(${msg.id})">Dismiss</button>
                    </div>
                </div>`;
        } else {
            const rolesHtml = (tmpl.roles || []).map(r => `<span class="session-role-pill">${escapeHtml(r)}</span>`).join(' ');
            const phases = tmpl.phases || [];
            const phasesDetailHtml = phases.map((p, i) => {
                const parts = (p.participants || []).map(r => escapeHtml(r)).join(', ');
                const promptText = p.prompt ? escapeHtml(p.prompt) : '';
                return `<div class="session-draft-phase-detail">
                    <span class="session-draft-phase-num">${i + 1}.</span>
                    <span class="session-draft-phase-name">${escapeHtml(p.name)}${p.is_output ? ' <em>(output)</em>' : ''}</span>
                    ${parts ? `<span class="session-draft-phase-parts">${parts}</span>` : ''}
                    ${promptText ? `<div class="session-draft-phase-prompt">${promptText}</div>` : ''}
                </div>`;
            }).join('');
            const phasesSummary = phases.map(p => escapeHtml(p.name)).join(' -> ');
            el.dataset.draftTemplate = JSON.stringify(tmpl);
            el.dataset.draftMsgId = msg.id;
            el.innerHTML = `
                <div class="session-draft-body">
                    <div class="session-draft-compact">
                        <div class="session-draft-header">${escapeHtml(tmpl.name || '?')}</div>
                        ${tmpl.description ? `<div class="session-draft-desc">${escapeHtml(tmpl.description)}</div>` : ''}
                        <div class="session-draft-roles">${rolesHtml}</div>
                        <div class="session-draft-flow">${phasesSummary}</div>
                        <div class="session-draft-meta">by ${escapeHtml(proposedBy)} | rev ${revision}</div>
                    </div>
...
function switchChannel(name) {
    ...
    filterMessagesByChannel();
    renderChannelTabs();
    activeSession = activeSessionsByChannel[name] || null;
    updateSessionBar();
    fetchActiveSession(name);
...
async function fetchSessionTemplates() {
    try {
        const res = await fetch('/api/sessions/templates', { headers: { 'X-Session-Token': SESSION_TOKEN } });
...
async function fetchAllActiveSessions() {
    try {
        const res = await fetch('/api/sessions/active-all', { headers: { 'X-Session-Token': SESSION_TOKEN } });
        if (res.ok) {
            ...
            activeSession = activeSessionsByChannel[activeChannel] || null;
            updateSessionBar();
        }
    } catch (e) {
...
async function fetchActiveSession(channelName = activeChannel) {
    try {
        const res = await fetch(`/api/sessions/active?channel=${encodeURIComponent(channelName)}`, { headers: { 'X-Session-Token': SESSION_TOKEN } });
        if (res.ok) {
...
function handleSessionEvent(action, session) {
    ...
    if (action === 'create' || action === 'update') {
        ...
        if (channel === activeChannel && action === 'complete' && session.output_message_id) {
...
function updateSessionBar() {
    ...
    if (!activeSession) {
        const otherSessions = Object.values(activeSessionsByChannel)
            .filter(session => session && (session.channel || 'general') !== activeChannel);
...
function jumpToSessionChannel() {
    if (!sessionIndicatorTargetChannel || sessionIndicatorTargetChannel === activeChannel) return;
    switchChannel(sessionIndicatorTargetChannel);
}
    ...
    if (!phases.length) {
        return (tmpl.roles || []).map(role => {
            ...
            const options = assignees.map(a =>
                `<option value="${escapeHtml(a)}" ${a === assigned ? 'selected' : ''}>${escapeHtml(a)}</option>`
            ).join('');
            return `<div class="session-cast-row">
                <span class="session-cast-role">${escapeHtml(role)}</span>
                <select class="session-cast-select" data-role="${escapeHtml(role)}" onchange="syncSessionCastRole(this)">${options}</select>
            </div>`;
        }).join('');
    }
    ...
    return phases.map((phase, idx) => {
        ...
        return `<div class="session-cast-phase">
            <div class="session-cast-phase-head">
                ...
                <span class="session-cast-phase-name">${escapeHtml(phase.name || `Phase ${idx + 1}`)}</span>
            </div>
...
function showSessionLauncher() {
    ...
    let templateOptions = sessionTemplates.map(t =>
        `<div class="session-tmpl-card" onclick="showCastPreview('${escapeHtml(t.id)}')" title="${escapeHtml(t.description || '')}">
            ${t.is_custom ? `<span class="session-tmpl-delete-wrap"><button class="session-tmpl-delete" onclick="toggleDeleteSessionTemplateConfirm(this, '${escapeHtml(t.id)}', event)" title="Delete custom template">Delete</button></span>` : ''}
            <div class="session-tmpl-name">${escapeHtml(t.name)}</div>
            <div class="session-tmpl-desc">${escapeHtml(t.description || '')}</div>
            <div class="session-tmpl-roles">${(t.roles || []).map(r => `<span class="session-role-pill">${escapeHtml(r)}</span>`).join(' ')}</div>
        </div>`
    ).join('');
...
async function sendDesignRequest() {
    ...
    try {
        await fetch('/api/sessions/request-draft', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-Session-Token': SESSION_TOKEN },
            body: JSON.stringify({ agent: agent, description: desc, channel: activeChannel, sender: username }),
        });
    } catch (e) {
...
function showCastPreview(templateId) {
    ...
    const assignees = [...agents, username];
    ...
    castStep.innerHTML = `
        <div class="session-cast-header">
            ...
            <span>${escapeHtml(tmpl.name)} -- Cast</span>
        </div>
        <div class="session-cast-list">${roleRows}</div>
        <button class="session-start-btn" onclick="launchSessionWithCast('${escapeHtml(templateId)}')">Start Session</button>
    `;
}
...
function requestDraftChanges(draftId, proposedBy, msgId) {
    ...
    inputRow.innerHTML = `
        ...
        <div class="draft-changes-btns">
            <button class="session-draft-btn run" onclick="submitDraftChanges('${escapeHtml(draftId)}', '${escapeHtml(proposedBy)}', ${msgId})">Send</button>
